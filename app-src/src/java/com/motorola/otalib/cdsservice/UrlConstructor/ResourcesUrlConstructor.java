...
public class ResourcesUrlConstructor {
    public static String constructUrl(UrlRequest urlRequest) {
        if (!TextUtils.isEmpty(urlRequest.getTestUrl()) && BuildPropertyUtils.isDogfoodDevice()) {
            CDSLogger.v(CDSLogger.TAG, "Resource Request test url is " + urlRequest.getTestUrl());
        ...
        CDSLogger.v(CDSLogger.TAG, "Resources Request getUrl(): url is " + sb.toString());
        return sb.toString();
    }
}
